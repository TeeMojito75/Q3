...
int dfs (int n, int m, const Point& p, vector< vector<char> >& map, 
    vector< vector<bool> >& marked, const int N_DIRS, const int DR[], const int DC[]) {
    int s = 0;
    marked[p.r][ p.c] = true;
...
int main(void) {
    ...
    while (cin >> n >> m) {
        ...
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < m; ++j) {
            cin >> map[i][j];
            if (map[i][j] == 'K') knights.push_back(Point(i, j));
            else if (map[i][j] == 'B') bishops.push_back(Point(i, j));
        }
